          ...
          <>
            {addressData.map((info, i) => (
              <div className="myaddress" key={info.id}>
                <h4>{info.name}</h4>
                <span>
                  {info.flatDetails}, {info.areaDetails}, {info.townCity},{" "}
                  {info.state}
                </span>
                {info.landmark && (
                  <p>
                ...
                <p>
                  Contact Number:{" "}
                  {info.contactNo.startsWith("91")
                    ? info.contactNo.substring(2)
                    : info.contactNo}
                </p>
                {info.isDefault && (
                  <h3 className="default-address">Default Address</h3>
                )}
                <div className="address-action">
                  <button onClick={() => handleEdit(i)}>Edit</button>
                  <button onClick={() => handleRemoveAddress(info.id)}>
                    Remove
                  </button>
                  {!info.isDefault && (
                    <button onClick={() => handleSetDefault(info.id)}>
                      Set as Default
                    </button>
                  )}
                </div>
